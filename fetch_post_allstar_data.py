...
# Fetch all teams
# ----------------------------
print(f"Fetching NBA post All-Star data for {SEASON} season...")
...
for t in pbar:
    ...
    try:
        ...
        # Filter to post All-Star games only
        df = df[df["GAME_DATE"] >= CUTOFF_DATE].reset_index(drop=True)

        games = len(df)
        if games < WINDOW:
            pbar.write(f"[SKIP] {team_name}: only {games} post-ASB games (need {WINDOW})")
            continue
...
if len(teams_data) > 0:
    ...
    def_slopes = [-t["defSlope"] for t in teams_data]
    ...
    for i, team in enumerate(teams_data):
        ...
        def_high = 1 if def_level <= def_median else 0
        off_consistent = 1 if off_rug <= off_rug_median else 0
        ...
        def_slope_z = -(def_slope - def_slope_median) / def_slope_std if def_slope_std > 0 else 0
...
print(f"  {len(teams_data)} teams, {sum(t['games'] for t in teams_data)} total post-ASB games")
